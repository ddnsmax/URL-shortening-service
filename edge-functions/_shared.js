...
function resolveKvBinding(context) {
  const legacy = context && context.env && context.env.duanlianjie || globalThis && globalThis.duanlianjie || null;
  const blob = getStore({ name: 'duanlianjie', consistency: 'strong' });
  return {
    async get(key) {
      const value = await blob.get(key, { consistency: 'strong' });
      if (value !== null) return value;
      if (!legacy) return null;
      const legacyValue = await legacy.get(key);
      if (legacyValue !== null && legacyValue !== undefined) await blob.set(key, String(legacyValue), { cacheControl: 'no-store' });
      return legacyValue;
    },
    async put(key, value) {
      await blob.set(key, String(value), { cacheControl: 'no-store' });
    },
    async delete(key) {
      await blob.delete(key);
      if (legacy) await legacy.delete(key);
    }
  };
}
