...
function qx11(ctx, kv, host, path) {
  try {
    if (!ctx || typeof ctx.waitUntil !== 'function' || !kv) return;
    ctx.waitUntil(qx12(kv, host, path).catch(() => {}));
  } catch (e) {}
}

async function qx12(kv, host, path) {
  const h = qx9(host);
  if (!h) return;
...
function createD1Store(db) {
  ...
  return {
    async get(key) {
      ...
      if (key === GLOBAL_ACCESS_KEY) return await getConfigValue(key);
      if (key === 'shortlink_project_id') return await getConfigValue(key);
      if (key.startsWith('domain_report:')) return await getConfigValue(key);
      return null;
    },
    async put(key, value) {
      ...
      if (key === GLOBAL_ACCESS_KEY) return await putConfigValue(key, value);
      if (key === 'shortlink_project_id') return await putConfigValue(key, value);
      if (key.startsWith('domain_report:')) return await putConfigValue(key, value);
    },
    async delete(key) {
      ...
      if (key === GLOBAL_ACCESS_KEY) return await deleteConfigValue(key);
      if (key === 'shortlink_project_id') return await deleteConfigValue(key);
      if (key.startsWith('domain_report:')) return await deleteConfigValue(key);
    }
  };
}
