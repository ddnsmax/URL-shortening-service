...
function createD1Store(db) {
  ...
  async function listLinks(status) {
    const statement = status
      ? db.prepare('SELECT short, long_url, status, created_at, approved_at, visits, last_visited_at, is_permanent FROM links WHERE status = ? ORDER BY created_at DESC').bind(status)
      : db.prepare('SELECT short, long_url, status, created_at, approved_at, visits, last_visited_at, is_permanent FROM links ORDER BY created_at DESC');
    const result = await statement.all();
    const rows = result && result.results ? result.results : [];
    return rows.map(row => ({ short: row.short, ...JSON.parse(rowToLink(row)) }));
  }
...
async function updateLinkLongUrl(kv, payload) {
  const short = String(payload.short || '').trim();
  const longUrl = normalizeHttpUrl(payload.longUrl);
  if (!short || !longUrl) return textResponse('请输入正确的 http 或 https 链接', 400);
  const linkStr = await kv.get('short_link:' + short);
  if (!linkStr) return textResponse('链接不存在', 404);
  const linkData = JSON.parse(linkStr);
  linkData.longUrl = longUrl;
  await kv.put('short_link:' + short, JSON.stringify(linkData));
  return jsonResponse({ status: 'ok', link: { short, ...linkData } });
}
...
async function loadAdminLinks(kv, config, view) {
  const statusFilter = view === 'manage' ? 'approved' : view === 'audit' ? 'pending' : null;
  if (view && !statusFilter) return [];
  const storedLinks = typeof kv.listLinks === 'function' ? await kv.listLinks(statusFilter) : null;
  const items = storedLinks || await getLinkKeys(kv);
  const now = Date.now();
  const approvedCutoff = now - normalizeCleanupDays(config.auto_clean_days, 30) * 86400000;
  const pendingCutoff = now - normalizeCleanupDays(config.pending_auto_clean_days, 30) * 86400000;
  const records = await mapWithConcurrency(items, 8, async item => {
    try {
      const key = typeof item === 'string' ? item : item.short;
      const link = typeof item === 'string' ? JSON.parse(await kv.get('short_link:' + key)) : item;
      if (!link) return null;
      const approvedExpired = config.auto_clean_enabled === 1 && link.status === 'approved' && !link.isPermanent && Number(link.lastVisitedAt || link.createdAt) < approvedCutoff;
      const pendingExpired = config.pending_auto_clean_enabled === 1 && link.status === 'pending' && Number(link.createdAt) > 0 && Number(link.createdAt) <= pendingCutoff;
      if (approvedExpired || pendingExpired) {
        await kv.delete('short_link:' + key);
        return { removed: key };
      }
      return { link: { short: key, ...link } };
    } catch (error) {
      return null;
    }
  });
  const removed = records.filter(record => record && record.removed).map(record => record.removed);
  return records
    .filter(record => record && record.link && (!statusFilter || record.link.status === statusFilter))
...
async function handleRequest(context) {
  ...
  if (path.startsWith(config.adminPath)) {
    ...
    if (path === config.adminPath + '/api/data') {
      const links = await loadAdminLinks(kv, config, url.searchParams.get('view'));
    ...
    if (path === config.adminPath + '/api/action' && request.method === 'POST') {
      ...
      if (action === 'update_basic_config') {
        ...
        return jsonResponse({ status: 'ok', link: { short: payload.short, ...linkData } });
      } 
      else if (action === 'reject' || action === 'delete') {
        await kv.delete('short_link:' + payload.short);
        await removeLinkKey(kv, payload.short);
        return jsonResponse({ status: 'ok', deleted: payload.short });
      }
        ...
        return await updateLinkLongUrl(kv, payload);
      }
      return textResponse('ok', 200);
    }
...
function getAdminHtml(path, currentHost) {
  return enhanceAdminHtml(getAdminHtmlBase(path, currentHost));
}

function enhanceAdminHtml(html) {
  let enhancedHtml = html;
  enhancedHtml = enhancedHtml.replace(
    "let allLinks=[];let otpSecretTemp='';",
    "let allLinks=[];let otpSecretTemp='';let dataRequestId=0;function currentTab(){const active=document.querySelector('.nav-item.active');return active?active.dataset.tab:'basic';}async function fetchAdminData(tab,requestId){try{const response=await fetch(API_BASE+'/data?view='+encodeURIComponent(tab)+'&_='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-cache'}});if(!response.ok)throw new Error(await response.text()||'数据加载失败');const data=await response.json();return requestId===dataRequestId?data:null;}catch(error){if(requestId===dataRequestId)alert(error.message||'数据加载失败');return null;}}async function checkedFetch(input,init){const response=await fetch(input,init);if(!response.ok){const message=await response.clone().text();alert(message||'操作失败');throw new Error(message||'操作失败');}return response;}async function postAction(action,payload){const response=await fetch(API_BASE + '/action',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,payload})});const raw=await response.text();let result=null;try{result=raw?JSON.parse(raw):null;}catch(error){}if(!response.ok)throw new Error(result&&result.message||raw||'操作失败');return result||{status:'ok'};}function applyActionResult(action,short,result){if(result.deleted||action==='delete'||action==='reject'){allLinks=allLinks.filter(link=>link.short!==short);renderTables();return;}if(!result.link)return;const active=currentTab();const visible=active==='manage'?result.link.status==='approved':active==='audit'?result.link.status==='pending':true;const index=allLinks.findIndex(link=>link.short===short);if(!visible){if(index>=0)allLinks.splice(index,1);}else if(index>=0){allLinks[index]=result.link;}else{allLinks.unshift(result.link);}renderTables();}"
  );
  enhancedHtml = enhancedHtml.replace(
    "function switchTab(id){document.querySelectorAll('.nav-item').forEach(e=>e.classList.remove('active'));document.querySelectorAll('.tab-pane').forEach(e=>e.classList.remove('active'));document.querySelector('[data-tab=\"'+id+'\"]').classList.add('active');document.getElementById('tab-'+id).classList.add('active');if(history.pushState){history.pushState(null,null,'#'+id)}else{window.location.hash='#'+id}}",
    "function switchTab(id,reload=true){let nav=document.querySelector('[data-tab=\"'+id+'\"]');let pane=document.getElementById('tab-'+id);if(!nav||!pane){id='basic';nav=document.querySelector('[data-tab=\"basic\"]');pane=document.getElementById('tab-basic');}document.querySelectorAll('.nav-item').forEach(element=>element.classList.remove('active'));document.querySelectorAll('.tab-pane').forEach(element=>element.classList.remove('active'));nav.classList.add('active');pane.classList.add('active');if(history.pushState){history.pushState(null,null,'#'+id)}else{window.location.hash='#'+id}if(reload)loadData(id);}"
  );
  enhancedHtml = enhancedHtml.replace(
    "document.addEventListener('DOMContentLoaded',()=>{const hash=window.location.hash.replace('#','')||'basic';switchTab(hash);loadData();});",
    "document.addEventListener('DOMContentLoaded',()=>{const hash=window.location.hash.replace('#','')||'basic';switchTab(hash,false);loadData(hash);});"
  );
  enhancedHtml = enhancedHtml.replace(
    "async function loadData(){const r=await fetch(API_BASE+'/data');const d=await r.json();allLinks=d.links;",
    "async function loadData(tab=currentTab()){const requestId=++dataRequestId;const d=await fetchAdminData(tab,requestId);if(!d)return;allLinks=Array.isArray(d.links)?d.links:[];"
  );
  enhancedHtml = enhancedHtml.replace(
    "async function doAction(a,s,p={}){await fetch(API_BASE+'/action',{method:'POST',body:JSON.stringify({action:a,payload:{short:s,...p}})});loadData();}",
    "async function doAction(action,short,payload={}){try{const result=await postAction(action,{short,...payload});applyActionResult(action,short,result);}catch(error){alert(error.message||'操作失败');}}"
  );
  enhancedHtml = enhancedHtml.replace(
    "function confirmDelete(short){",
    "async function editLink(short){const link=allLinks.find(item=>item.short===short);if(!link)return alert('链接不存在');const input=prompt('输入新的原长链接：',link.longUrl||'');if(input===null)return;const longUrl=input.trim();let parsed=null;try{parsed=new URL(longUrl);}catch(error){}if(!parsed||(parsed.protocol!=='http:'&&parsed.protocol!=='https:'))return alert('请输入正确的 http 或 https 链接');if(longUrl===link.longUrl)return;await doAction('edit_link',short,{longUrl});}function confirmDelete(short){"
  );
  const deleteButton = `<button class="btn btn-danger" style="padding:6px 12px;font-size:12px" onclick="confirmDelete(&quot;'+safeShort+'&quot;)">删除</button>`;
  const editButton = `<button class="btn" style="padding:6px 12px;font-size:12px" onclick="editLink(&quot;'+safeShort+'&quot;)">编辑</button>`;
  enhancedHtml = enhancedHtml.replace(deleteButton, editButton + deleteButton);
  enhancedHtml = enhancedHtml.split("fetch(API_BASE+'/action'").join("checkedFetch(API_BASE+'/action'");
  return enhancedHtml;
}

function getAdminHtmlBase(path, currentHost) {
